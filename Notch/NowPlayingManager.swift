...
class NowPlayingManager: ObservableObject {
    ...
    func skipBackward() {
        ...
        // ⚡️ THE RESTART FIX: Smart JS injector specifically for the previous button
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            // If we are < 3 seconds in, physically skip to the previous track
    ...
    func togglePlayPause() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if let browser = self.lastActiveBrowser, let wIdx = self.lastWindowIndex, let tIdx = self.lastTabIndex {
                ...
                if let res = NSAppleScript(source: fastScript)?.executeAndReturnError(nil).stringValue, (res == "PLAYED" || res == "PAUSED") {
                    DispatchQueue.main.async { self.isPlaying = (res == "PLAYED") }
                    self.triggerFastFetch()
                    return
                }
            }
