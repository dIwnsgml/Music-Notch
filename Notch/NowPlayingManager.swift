...
class NowPlayingManager: ObservableObject {
    ...
    func seek(to percentage: Double) {
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                // ⚡️ BUG FIX: Pure Math Gapless Offset Calculation for YouTube Music
    ...
    func fetchTitle() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            // ⚡️ BUG FIX: Force reading `cCurr` from the UI text on YTM to entirely bypass gapless buffer times!
    ...
    private func parseAndApplyResult(result: String, browser: String) {
        DispatchQueue.main.async {
            ...
            if self.internalSongIdentifier != identifier {
                ...
                // ⚡️ BUG FIX: Force reset the timer the exact millisecond the track changes
                self.currentTime = 0.0
            }
